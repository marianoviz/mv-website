---
title: "Seasonality Trends of Coho, Jack Coho Salmon, and Steelhead Trout in the Columbia River Basin (2000-2010)"
description: |
  In this analysis, we explored the seasonal migration trends of Steelhead trout, Coho salmon, and Jack-Coho salmon as they passed through the Willamette Falls fish ladder in Oregon, using data from 2000 to 2010. The analysis involved time series visualization to capture the temporal trends for each species and seasonal plots to illustrate the variation within each year. The findings revealed distinct seasonal migratory patterns, with all three species showing peak presence in the fall. The time series analysis indicated that Steelhead trout numbers remained relatively stable, while Coho and Jack-Coho salmon populations displayed an increasing trend over the studied period.
author:
...
![](https://images.squarespace-cdn.com/content/v1/5c33c4d3b1059881395762dd/1684275844379-UL9LC5Z08DOODPNADMNR/WFT_20220714_281_IL+3.jpg?format=2500w)
...
![](https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEhPDP20xSyNAtT5U6FJNzzzSCnmGecCWGUKVE4MjBnTKFA4amUWtgvbrf4aZOb_ktZGp9Ckm-mkxIiy32Kj5g_30pmOil5NbVaQLUQ_8GW1BKWZAQVFzH1XqOq6ibtnKaCCdVTyq4uVj6s/s1600/Salmon+Map.jpg)
...
```{r}
# Read in data
fish_data <- read_csv("willamette_fish_passage.csv") %>% 
  clean_names()
...
```{r, fig.cap="This time series reveals the trend in steelhead salmon migration through the Columbia River from 2000 to 2010.Data: [Columbia River DART](http://www.cbr.washington.edu/dart/query/adult_graph_text) (Data Access in Real Time)."}
# Steelhead salmon
...
```{r, fig.cap="This time series reveals the trend in coho salmon migration through the Columbia River from 2000 to 2010.Data: [Columbia River DART](http://www.cbr.washington.edu/dart/query/adult_graph_text) (Data Access in Real Time)."}
# Coho slamon
...
```{r, fig.cap="This time series reveals the trend in jack coho salmon migration through the Columbia River from 2000 to 2010.Data: [Columbia River DART](http://www.cbr.washington.edu/dart/query/adult_graph_text) (Data Access in Real Time)."}
# Jack Coho salmon
...
- All three species have strong, seasonal migratory patterns through the Columbia River. Each species appears to have a strong presence later in the year - during the fall.

- Steelhead trout numbers have remained relatively constant and are predicted to remain constant through 2030 while coho and jack coho salmon numbers are expected to increase.

- Steelhead trout spend a greater time on average in the Columbia River than do either of the other two species. 

### Tab 2: Season Plots 


```{r, fig.width = 10, fig.height = 8,fig.cap="Abundance of Coho, Jack-Coho and Steelhead seasonally observed in at Willamette Falls fish ladder Willamette, Oregon. These observations range from 2001-2010.Year is indicated by color. Data: [Columbia River DART](http://www.cbr.washington.edu/dart/query/adult_graph_text) (Data Access in Real Time)."}
...
patch_season <- (coho_season/jack_coho_season/steelhead_season)+
  plot_annotation(title = "Coho, Jack-Coho, and Steelhead Seasonal Abundance in Willamette Falls (2001-2010)")

patch_season
```


From the season plots on steelhead trout, coho and jack coho salmon, we can see that:
...
```{r, fig.cap="Annual counts by species at the Willamette Falls fish ladder, Willamette River, Oregon (2001-2010). Red, orange, and green lines indicate counts for the species coho, jack coho, and steelhead, respectively. Data: [Columbia River DART](http://www.cbr.washington.edu/dart/query/adult_graph_text) (Data Access in Real Time)."}
